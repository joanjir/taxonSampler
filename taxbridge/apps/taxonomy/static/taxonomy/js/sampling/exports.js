...
INPUT_FILE="\${WORK_DIR}/accessions.txt"
ZIP_FILE="\${WORK_DIR}/download_dehydrated.zip"
UNZIP_DIR="\${WORK_DIR}/data"
LOG_FILE="\${WORK_DIR}/download.log"
...
case "$(uname -s)" in
  ...
  MINGW*|MSYS*|CYGWIN*)
    ...
    ;;
  *) echo "ERROR: OS no reconocido: $(uname -s)"; exit 1 ;;
esac

# ── Install datasets CLI if not present ────────────────────
if ! command -v datasets >/dev/null 2>&1 && [[ ! -x "\${DATASETS_BIN}" ]]; then
  echo "datasets CLI not found. Installing from \${DL_URL}..."
  if ! curl -fSL -o "\${DATASETS_BIN}" "\${DL_URL}"; then
    rm -f "\${DATASETS_BIN}"
    echo ""
    echo "ERROR: Could not download datasets CLI."
    echo "Install it manually from: https://www.ncbi.nlm.nih.gov/datasets/docs/v2/download-and-install/"
    echo "Then place the binary in: \${BASE_DIR}/"
    exit 1
  fi
  # Verify it's a real binary, not an error page
  if file "\${DATASETS_BIN}" | grep -qi "text\\|xml\\|html"; then
    rm -f "\${DATASETS_BIN}"
    echo ""
    echo "ERROR: Downloaded file is not a valid binary (got HTML/XML error page)."
    echo "Install datasets CLI manually from: https://www.ncbi.nlm.nih.gov/datasets/docs/v2/download-and-install/"
    exit 1
  fi
  chmod +x "\${DATASETS_BIN}" 2>/dev/null || true
  echo "datasets installed at \${DATASETS_BIN}"
fi
...
echo "================================================"
echo "[$(date '+%F %T')] Total accessions: \${TOTAL}" | tee -a "\${LOG_FILE}"

# ── Download (dehydrated) ─────────────────────────────────
echo "[$(date '+%F %T')] Starting batch download..." | tee -a "\${LOG_FILE}"
...
if [[ ! -s "\${ZIP_FILE}" ]]; then
  echo "Error: ZIP empty or not created: \${ZIP_FILE}" | tee -a "\${LOG_FILE}"
  exit 1
fi

echo "[$(date '+%F %T')] ZIP downloaded: \${ZIP_FILE}" | tee -a "\${LOG_FILE}"

# ── Unzip + rehydrate ─────────────────────────────────────
echo "[$(date '+%F %T')] Extracting..." | tee -a "\${LOG_FILE}"
unzip -o "\${ZIP_FILE}" -d "\${UNZIP_DIR}" >> "\${LOG_FILE}" 2>&1

echo "[$(date '+%F %T')] Rehydrating files..." | tee -a "\${LOG_FILE}"
"\${DATASETS_CMD}" rehydrate \\
  --directory "\${UNZIP_DIR}" \\
  2>&1 | tee -a "\${LOG_FILE}"
...
echo "Directory: \${UNZIP_DIR}"
echo "Data files:"
find "\${UNZIP_DIR}" -type f \\( -name "*.fna" -o -name "*.fna.gz" -o -name "*.faa" -o -name "*.faa.gz" -o -name "*.gbff" -o -name "*.gbff.gz" \\) | wc -l
echo "================================================"
`;
  }
...
$InputFile = Join-Path $WorkDir "accessions.txt"
$ZipFile   = Join-Path $WorkDir "download_dehydrated.zip"
$UnzipDir  = Join-Path $WorkDir "data"
$LogFile   = Join-Path $WorkDir "download.log"
$DatasetsBin = Join-Path $ScriptDir "datasets.exe"

New-Item -ItemType Directory -Path $WorkDir -Force | Out-Null

# ── Install datasets CLI if not present ────────────────────
if (-not (Get-Command datasets -ErrorAction SilentlyContinue) -and -not (Test-Path $DatasetsBin)) {
  Write-Host "datasets CLI not found. Installing..."
  ...
  Write-Host "datasets installed at $DatasetsBin"
}

if (Get-Command datasets -ErrorAction SilentlyContinue) {
  $DatasetsCmd = "datasets"
} else {
...
$Total = (Get-Content $InputFile | Where-Object { $_.Trim() -ne "" }).Count
Write-Host ""
Write-Host "TaxonSampler - NCBI ${label} Download"
Write-Host "================================================"
Write-Host "  Species: $Total"
Write-Host "  Output:  $WorkDir"
Write-Host "================================================"

$ts = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
"[$ts] Total accessions: $Total" | Tee-Object -FilePath $LogFile -Append

# ── Download (dehydrated) ─────────────────────────────────
$ts = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
"[$ts] Starting batch download..." | Tee-Object -FilePath $LogFile -Append

& $DatasetsCmd download genome accession \`
  --inputfile $InputFile \`
  --include ${cliInclude} \`
  --dehydrated \`
  --filename $ZipFile 2>&1 | Tee-Object -FilePath $LogFile -Append

if (-not (Test-Path $ZipFile) -or (Get-Item $ZipFile).Length -eq 0) {
  Write-Host "Error: ZIP empty or not created: $ZipFile"
  exit 1
}

$ts = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
"[$ts] ZIP downloaded: $ZipFile" | Tee-Object -FilePath $LogFile -Append

# ── Unzip + rehydrate ─────────────────────────────────────
$ts = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
"[$ts] Extracting..." | Tee-Object -FilePath $LogFile -Append
Expand-Archive -Path $ZipFile -DestinationPath $UnzipDir -Force

$ts = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
"[$ts] Rehydrating files..." | Tee-Object -FilePath $LogFile -Append
& $DatasetsCmd rehydrate --directory $UnzipDir 2>&1 | Tee-Object -FilePath $LogFile -Append

# ── Summary ───────────────────────────────────────────────
Write-Host ""
$ts = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
"[$ts] Done." | Tee-Object -FilePath $LogFile -Append
Write-Host "Directory: $UnzipDir"
Write-Host "Data files:"
(Get-ChildItem -Path $UnzipDir -Recurse -Include *.fna,*.fna.gz,*.faa,*.faa.gz,*.gbff,*.gbff.gz).Count
Write-Host "================================================"
`;
  }
