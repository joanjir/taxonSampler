...
export function initExportHandlers({ getExportPayload, getLastSampling }) {
  ...
  // Uses NCBI datasets CLI with dehydrated download + rehydrate
  ...
  // Map includeFlag to datasets CLI --include values
  const CLI_INCLUDE_MAP = { genome: "genome", protein: "protein", gbff: "gbff" };
  function _cliIncludeArg(includeFlag) {
    return includeFlag.split(",").map(f => CLI_INCLUDE_MAP[f.trim()]).filter(Boolean).join(",");
  }

  // Map includeFlag → file extensions for find/Get-ChildItem
  const FILE_EXT_MAP = {
    genome: ["*.fna", "*.fna.gz"],
    protein: ["*.faa", "*.faa.gz"],
    gbff: ["*.gbff", "*.gbff.gz"],
  };
  function _findPatternBash(includeFlag) {
    const exts = includeFlag.split(",").flatMap(f => FILE_EXT_MAP[f.trim()] || []);
    if (!exts.length) return '-name "*.fna" -o -name "*.fna.gz"';
    return exts.map(e => `-name "${e}"`).join(" -o ");
  }
  function _findPatternPs(includeFlag) {
    const exts = includeFlag.split(",").flatMap(f => FILE_EXT_MAP[f.trim()] || []);
    return exts.length ? exts.join(",") : "*.fna,*.fna.gz";
  }
  ...
  function buildNcbiScript(species, includeFlag, label, outZip) {
    ...
    const cliInclude = _cliIncludeArg(includeFlag);
    const findPattern = _findPatternBash(includeFlag);
    return `#!/usr/bin/env bash
set -Eeuo pipefail

# ============================================================
# NCBI ${label} batch downloader — Generated by TaxonSampler
# Date: ${now} | Species: ${accessions.length} | Data: ${includeFlag}
# - Uses official datasets CLI
# - Downloads in chunks (resumes if interrupted)
# - Dehydrated + rehydrate workflow
# - Consolidates output files
# ============================================================

# ── Configuration ─────────────────────────────────────────
BASE_DIR="$(cd "$(dirname "\${BASH_SOURCE[0]}")" && pwd)"
WORK_DIR="\${BASE_DIR}/${folder}"
DATASETS_BIN="\${BASE_DIR}/datasets"

ACCESSIONS_FILE="\${WORK_DIR}/accessions.txt"
UNIQUE_FILE="\${WORK_DIR}/accessions.unique.txt"
CHUNKS_DIR="\${WORK_DIR}/chunks"
DOWNLOADS_DIR="\${WORK_DIR}/downloads"
REHYDRATED_DIR="\${WORK_DIR}/rehydrated"
...
cleanup_on_error() {
  local exit_code=\$?
  log "Failed at line \${BASH_LINENO[0]} with code \${exit_code}"
  exit "\${exit_code}"
}
...
# ── Create directories ────────────────────────────────────
mkdir -p "\${WORK_DIR}" "\${CHUNKS_DIR}" "\${DOWNLOADS_DIR}" "\${REHYDRATED_DIR}" "\${MERGED_DIR}" "\${LOG_DIR}"
touch "\${MASTER_LOG}"
...
# ── Detect OS / platform ─────────────────────────────────
case "$(uname -s)" in
  Linux*)  DL_URL="https://ftp.ncbi.nlm.nih.gov/pub/datasets/command-line/v2/linux-amd64/datasets" ;;
  Darwin*) DL_URL="https://ftp.ncbi.nlm.nih.gov/pub/datasets/command-line/v2/mac/datasets" ;;
  MINGW*|MSYS*|CYGWIN*)
    DL_URL="https://ftp.ncbi.nlm.nih.gov/pub/datasets/command-line/v2/win-amd64/datasets.exe"
    DATASETS_BIN="\${BASE_DIR}/datasets.exe"
    ;;
  *) die "Unrecognized OS: $(uname -s)" ;;
esac

# ── Install datasets CLI if not present ───────────────────
if command -v datasets >/dev/null 2>&1; then
  DATASETS_CMD="$(command -v datasets)"
elif [[ -x "\${DATASETS_BIN}" ]]; then
  DATASETS_CMD="\${DATASETS_BIN}"
else
  log "datasets not found. Installing..."
  if ! curl -fSL -o "\${DATASETS_BIN}" "\${DL_URL}"; then
    rm -f "\${DATASETS_BIN}"
    die "Could not download datasets CLI. Install manually from: https://www.ncbi.nlm.nih.gov/datasets/docs/v2/download-and-install/"
  fi
  if file "\${DATASETS_BIN}" 2>/dev/null | grep -qi "text\\|xml\\|html"; then
    rm -f "\${DATASETS_BIN}"
    die "Downloaded file is not a valid binary. Install manually from: https://www.ncbi.nlm.nih.gov/datasets/docs/v2/download-and-install/"
  fi
  chmod +x "\${DATASETS_BIN}" 2>/dev/null || true
  DATASETS_CMD="\${DATASETS_BIN}"
fi

[[ -x "\${DATASETS_CMD}" ]] || die "Could not prepare datasets CLI"

log "Using datasets at: \${DATASETS_CMD}"
"\${DATASETS_CMD}" version 2>&1 | tee -a "\${MASTER_LOG}"

# ── NCBI API key (optional — set yours for higher rate limits) ──
# export NCBI_API_KEY="your_api_key_here"

# ── Species manifest ─────────────────────────────────────
${manifest}

# ── Create accessions file (only if not present) ─────────
if [[ ! -f "\${ACCESSIONS_FILE}" ]]; then
cat > "\${ACCESSIONS_FILE}" <<'EOF'
${accessions.join("\n")}
EOF
  log "Accessions file created: \${ACCESSIONS_FILE}"
else
  log "Using existing accessions file: \${ACCESSIONS_FILE}"
fi

# ── Normalize and deduplicate ─────────────────────────────
awk 'NF {gsub(/^[ \\t]+|[ \\t]+$/, "", \$0); print \$0}' "\${ACCESSIONS_FILE}" \\
  | sort -u > "\${UNIQUE_FILE}"

TOTAL=$(wc -l < "\${UNIQUE_FILE}")
[[ "\${TOTAL}" -gt 0 ]] || die "No valid accessions in \${ACCESSIONS_FILE}"

log "Total unique accessions: \${TOTAL}"
...
split -l "\${CHUNK_SIZE}" -d -a 4 "\${UNIQUE_FILE}" "\${CHUNKS_DIR}/chunk_"

NUM_CHUNKS=$(find "\${CHUNKS_DIR}" -maxdepth 1 -type f -name 'chunk_*' | wc -l)
log "Chunks generated: \${NUM_CHUNKS} (size=\${CHUNK_SIZE})"
...
for chunk_file in "\${CHUNKS_DIR}"/chunk_*; do
  ...
  chunk_zip="\${DOWNLOADS_DIR}/\${chunk_name}.zip"
  chunk_unzip_dir="\${REHYDRATED_DIR}/\${chunk_name}"
  chunk_log="\${LOG_DIR}/\${chunk_name}.log"
  done_flag="\${chunk_unzip_dir}/.done"

  log "[\${chunk_index}/\${NUM_CHUNKS}] Processing \${chunk_name}"

  # ── Resume: skip if already done ──
  if [[ -f "\${done_flag}" ]]; then
    log "[\${chunk_index}/\${NUM_CHUNKS}] \${chunk_name} already completed, skipping"
    continue
  fi

  rm -f "\${chunk_zip}"
  rm -rf "\${chunk_unzip_dir}"
  mkdir -p "\${chunk_unzip_dir}"

  {
    echo "[\$(timestamp)] Downloading \${chunk_name}"
    ...
    echo "[\$(timestamp)] Extracting \${chunk_name}"
    unzip -o "\${chunk_zip}" -d "\${chunk_unzip_dir}"

    echo "[\$(timestamp)] Rehydrating \${chunk_name}"
    "\${DATASETS_CMD}" rehydrate --directory "\${chunk_unzip_dir}"

    touch "\${done_flag}"
    echo "[\$(timestamp)] OK \${chunk_name}"
  } >> "\${chunk_log}" 2>&1
...
log "Consolidating data files..."
find "\${REHYDRATED_DIR}" \\( ${findPattern} \\) -type f -print0 \\
| while IFS= read -r -d '' file; do
    cp -n "\$file" "\${MERGED_DIR}/"
done

FILE_COUNT=$(find "\${MERGED_DIR}" -type f \\( ${findPattern} \\) | wc -l)
DONE_COUNT=$(find "\${REHYDRATED_DIR}" -type f -name ".done" | wc -l)

log "Process complete"
log "Chunks completed: \${DONE_COUNT}/\${NUM_CHUNKS}"
log "Data files consolidated: \${FILE_COUNT}"
log "Consolidated directory: \${MERGED_DIR}"

echo ""
echo "Final Summary"
echo "─────────────────────────────────"
echo "  Unique accessions : \${TOTAL}"
echo "  Total chunks      : \${NUM_CHUNKS}"
echo "  Completed chunks  : \${DONE_COUNT}"
echo "  Data files found  : \${FILE_COUNT}"
...
`;
  ...
  function buildNcbiPsScript(species, includeFlag, label, outZip) {
    ...
    const folder = _folderName(includeFlag);
    const cliInclude = _cliIncludeArg(includeFlag);
    const psExtPattern = _findPatternPs(includeFlag);
    return `# ============================================================
# NCBI ${label} batch downloader (PowerShell) — Generated by TaxonSampler
# Date: ${now} | Species: ${accessions.length} | Data: ${includeFlag}
# - Uses official datasets CLI
# - Downloads in chunks (resumes if interrupted)
# - Dehydrated + rehydrate workflow
...
# ── Configuration ─────────────────────────────────────────
$WorkDir       = Join-Path $ScriptDir "${folder}"
$DatasetsBin   = Join-Path $ScriptDir "datasets.exe"

$AccessionsFile = Join-Path $WorkDir "accessions.txt"
$UniqueFile     = Join-Path $WorkDir "accessions.unique.txt"
$ChunksDir      = Join-Path $WorkDir "chunks"
$DownloadsDir   = Join-Path $WorkDir "downloads"
$RehydratedDir  = Join-Path $WorkDir "rehydrated"
$MergedDir      = Join-Path $WorkDir "merged"
$LogDir         = Join-Path $WorkDir "logs"
$MasterLog      = Join-Path $LogDir "master.log"

$ChunkSize = 50
$SleepBetweenChunks = 2

# ── Helper functions ──────────────────────────────────────
function ts { return (Get-Date -Format "yyyy-MM-dd HH:mm:ss") }
function Log($msg) { "[\$(ts)] \$msg" | Tee-Object -FilePath $MasterLog -Append }

# ── Create directories ────────────────────────────────────
foreach ($d in @($WorkDir, $ChunksDir, $DownloadsDir, $RehydratedDir, $MergedDir, $LogDir)) {
  New-Item -ItemType Directory -Path $d -Force | Out-Null
}
if (-not (Test-Path $MasterLog)) { New-Item $MasterLog -ItemType File -Force | Out-Null }

# ── Install datasets CLI if not present ───────────────────
if (Get-Command datasets -ErrorAction SilentlyContinue) {
  $DatasetsCmd = (Get-Command datasets).Source
} elseif (Test-Path $DatasetsBin) {
  $DatasetsCmd = $DatasetsBin
} else {
  Log "datasets not found. Installing..."
  $dlUrl = "https://ftp.ncbi.nlm.nih.gov/pub/datasets/command-line/v2/win-amd64/datasets.exe"
  try {
    Invoke-WebRequest -Uri $dlUrl -OutFile $DatasetsBin -UseBasicParsing
  } catch {
    Remove-Item $DatasetsBin -ErrorAction SilentlyContinue
    Write-Error "Could not download datasets CLI. Install manually from: https://www.ncbi.nlm.nih.gov/datasets/docs/v2/download-and-install/"
    exit 1
  }
  $DatasetsCmd = $DatasetsBin
}

Log "Using datasets at: $DatasetsCmd"
& $DatasetsCmd version 2>&1 | Tee-Object -FilePath $MasterLog -Append
...
# ── Create accessions file (only if not present) ─────────
if (-not (Test-Path $AccessionsFile)) {
@"
${accessions.join("\n")}
"@ | Set-Content -Path $AccessionsFile -Encoding UTF8
  Log "Accessions file created: $AccessionsFile"
} else {
  Log "Using existing accessions file: $AccessionsFile"
}

# ── Normalize and deduplicate ─────────────────────────────
Get-Content $AccessionsFile |
  Where-Object { $_.Trim() -ne "" } |
  ForEach-Object { $_.Trim() } |
  Sort-Object -Unique |
  Set-Content $UniqueFile -Encoding UTF8

$Total = (Get-Content $UniqueFile).Count
if ($Total -eq 0) { Write-Error "No valid accessions in $AccessionsFile"; exit 1 }
...
Write-Host "================================================"
Write-Host "  Accessions: $Total"
Write-Host "  Chunk size: $ChunkSize"
Write-Host "  Output:     $MergedDir"
Write-Host "================================================"

# ── Split into chunks ────────────────────────────────────
Get-ChildItem $ChunksDir -Filter "chunk_*" -ErrorAction SilentlyContinue | Remove-Item -Force
$lines = Get-Content $UniqueFile
$chunkIndex = 0
for ($i = 0; $i -lt $lines.Count; $i += $ChunkSize) {
  $chunk = $lines[$i..([Math]::Min($i + $ChunkSize - 1, $lines.Count - 1))]
  $chunkName = "chunk_{0:D4}" -f $chunkIndex
  $chunk | Set-Content (Join-Path $ChunksDir $chunkName) -Encoding UTF8
  $chunkIndex++
}
$NumChunks = $chunkIndex
Log "Chunks generated: $NumChunks (size=$ChunkSize)"

# ── Process chunks ────────────────────────────────────────
$chunkIdx = 0
foreach ($chunkFile in (Get-ChildItem $ChunksDir -Filter "chunk_*" | Sort-Object Name)) {
  $chunkIdx++
  $chunkName = $chunkFile.Name

  $chunkZip      = Join-Path $DownloadsDir "\$chunkName.zip"
  $chunkUnzipDir = Join-Path $RehydratedDir $chunkName
  $chunkLog      = Join-Path $LogDir "\$chunkName.log"
  $doneFlag      = Join-Path $chunkUnzipDir ".done"

  Log "[$chunkIdx/$NumChunks] Processing $chunkName"

  # ── Resume: skip if already done ──
  if (Test-Path $doneFlag) {
    Log "[$chunkIdx/$NumChunks] $chunkName already completed, skipping"
    continue
  }

  Remove-Item $chunkZip -ErrorAction SilentlyContinue
  if (Test-Path $chunkUnzipDir) { Remove-Item $chunkUnzipDir -Recurse -Force }
  New-Item -ItemType Directory -Path $chunkUnzipDir -Force | Out-Null

  try {
    "[\$(ts)] Downloading $chunkName" | Add-Content $chunkLog

    & $DatasetsCmd download genome accession \`
      --inputfile $($chunkFile.FullName) \`
      --include ${cliInclude} \`
      --dehydrated \`
      --filename $chunkZip 2>&1 | Add-Content $chunkLog

    if (-not (Test-Path $chunkZip) -or (Get-Item $chunkZip).Length -eq 0) {
      throw "ZIP empty or not created"
    }

    "[\$(ts)] Extracting $chunkName" | Add-Content $chunkLog
    Expand-Archive -Path $chunkZip -DestinationPath $chunkUnzipDir -Force

    "[\$(ts)] Rehydrating $chunkName" | Add-Content $chunkLog
    & $DatasetsCmd rehydrate --directory $chunkUnzipDir 2>&1 | Add-Content $chunkLog

    New-Item $doneFlag -ItemType File -Force | Out-Null
    "[\$(ts)] OK $chunkName" | Add-Content $chunkLog
  } catch {
    Log "[$chunkIdx/$NumChunks] FAILED $chunkName: $_"
  }

  Start-Sleep -Seconds $SleepBetweenChunks
}

# ── Consolidate output files ─────────────────────────────
Log "Consolidating data files..."
Get-ChildItem -Path $RehydratedDir -Recurse -Include ${psExtPattern} -File |
  ForEach-Object {
    $dest = Join-Path $MergedDir $_.Name
    if (-not (Test-Path $dest)) { Copy-Item $_.FullName $dest }
  }

$FileCount = (Get-ChildItem -Path $MergedDir -Include ${psExtPattern} -File).Count
$DoneCount = (Get-ChildItem -Path $RehydratedDir -Recurse -Filter ".done" -File).Count

Log "Process complete"
Log "Chunks completed: $DoneCount/$NumChunks"
Log "Data files consolidated: $FileCount"
Log "Consolidated directory: $MergedDir"

Write-Host ""
Write-Host "Final Summary"
Write-Host "-------------------------------------"
Write-Host "  Unique accessions : $Total"
Write-Host "  Total chunks      : $NumChunks"
Write-Host "  Completed chunks  : $DoneCount"
Write-Host "  Data files found  : $FileCount"
Write-Host "  Output directory  : $MergedDir"
